...
void Halo3BindClearanceProbe(uintptr_t base, size_t size)
{
    ...
    // Opt-in recovery candidate: regions include the retained safe pose.
    ...
    if (partitionsFlag && !kEnableHalo3WorldPartitionsExperiment)
        LOG("H3 world partitions EXPERIMENT disabled after Guardian repeat-approach recovery failure; prior contact path retained; VR unchanged");
