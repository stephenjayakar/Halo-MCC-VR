...
// update without leaving its Havok rigid body active. Apply a much smaller
// native point impulse first so Havok owns wakeup and angular response, then
// retain the bounded whole-body velocity correction. The fixed target-delta
// limit keeps authored mass in the wake request and makes the vehicle case
// over two orders of magnitude smaller than the rejected full point impulse.
