...
// Surface triangles do not report a hit when one closed solid is already
// contained inside another. Keep that case explicit: it can hold the rendered
// weapon outside a body, but its centre-derived normal is not trusted for a
// native physics impulse.
inline PhysicalContactCompoundHit PhysicalContactCompoundOverlap(
    const PhysicalContactCompoundShape& weapon,
    const PhysicalContactTransform& weaponTransform,
    const PhysicalContactCompoundShape& target,
    const PhysicalContactTransform& targetTransform)
{
    PhysicalContactCompoundHit result{};
    ...
    const auto worldCentroid = [](
        const PhysicalContactConvexShape& shape,
        const PhysicalContactTransform& transform) {
        PhysicalContactVec3 sum{};
        for (uint16_t vertex = 0; vertex < shape.vertexCount; ++vertex)
            sum = sum + PhysicalContactTransformPoint(
                transform, shape.vertices[vertex]);
        return sum * (1.0f / static_cast<float>(shape.vertexCount));
    };
    const PhysicalContactVec3 compoundFallback = PhysicalContactNormalize(
        PhysicalContactCompoundWorldCentroid(weapon, weaponTransform) -
            PhysicalContactCompoundWorldCentroid(target, targetTransform),
        PhysicalContactNormalize(
            weaponTransform.position - targetTransform.position,
         ...
         weaponChild < weapon.childCount; ++weaponChild)
    {
        ...
        {
            if (!PhysicalContactConvexIntersect(
                    weapon.children[weaponChild], weaponTransform,
                    target.children[targetChild], targetTransform))
                continue;
            result.hit = true;
            result.fraction = 1.0f;
            result.normalReliable = false;
            result.weaponChild = weaponChild;
            result.targetChild = targetChild;
            result.normal = PhysicalContactNormalize(
                worldCentroid(
                    weapon.children[weaponChild], weaponTransform) -
                    worldCentroid(
            ...
            result.targetPoint = PhysicalContactConvexSupport(
                ...
                result.normal);
            result.point =
                (result.weaponPoint + result.targetPoint) * 0.5f;
            return result;
        }
    }
    return result;
...
inline bool PhysicalContactCompoundsIntersect(
    ...
    const PhysicalContactTransform& targetTransform)
{
    return PhysicalContactCompoundOverlap(
        weapon, weaponTransform, target, targetTransform).hit;
}
...
// compounds instead. The normal is only a bounded search direction; the final
// translated pose is directly proven outside every solid child.
