...
struct PhysicalContactRigidMotionSample
{
    bool valid = false;
    PhysicalContactVec3 linearVelocity{};
    PhysicalContactVec3 angularVelocity{};
    PhysicalContactVec3 pivot{};
};

// Derive one rigid body's velocity from two exact world transforms. Animated
// Halo bodies expose the contacted limb transform directly, while the public
// object velocity accessor reports only the whole object's root.
inline PhysicalContactRigidMotionSample
PhysicalContactRigidMotionFromTransforms(
    const PhysicalContactTransform& previous,
    const PhysicalContactTransform& current,
    uint64_t previousMs, uint64_t currentMs, float worldUnitsPerMeter,
    float maximumAgeSeconds = 0.1f,
    float maximumLinearMetersPerSecond = 50.0f,
    float maximumAngularRadiansPerSecond = 50.0f)
{
    PhysicalContactRigidMotionSample result{};
    if (!PhysicalContactTransformFinite(previous) ||
        !PhysicalContactTransformFinite(current) || !previousMs ||
        currentMs <= previousMs ||
        !std::isfinite(worldUnitsPerMeter) || worldUnitsPerMeter <= 0.0f ||
        !std::isfinite(maximumAgeSeconds) || maximumAgeSeconds <= 0.0f ||
        maximumAgeSeconds > 0.5f ||
        !std::isfinite(maximumLinearMetersPerSecond) ||
        maximumLinearMetersPerSecond <= 0.0f ||
        !std::isfinite(maximumAngularRadiansPerSecond) ||
        maximumAngularRadiansPerSecond <= 0.0f ||
        std::fabs(previous.scale - current.scale) > 1.0e-3f)
        return result;
    const float elapsedSeconds =
        static_cast<float>(currentMs - previousMs) * 0.001f;
    ...
    const PhysicalContactVec3 linearVelocity =
        (current.position - previous.position) * (1.0f / elapsedSeconds);
    const float linearSpeedMeters =
        PhysicalContactLength(linearVelocity) / worldUnitsPerMeter;
    if (!PhysicalContactFinite(linearVelocity) ||
        !std::isfinite(linearSpeedMeters) ||
        linearSpeedMeters > maximumLinearMetersPerSecond)
        return result;
    ...
    if (!PhysicalContactFinite(sineAxisTwice) || !std::isfinite(sine) ||
        ...
        return result;
    PhysicalContactVec3 angularVelocity{};
    if (angle > 1.0e-6f && sine <= 1.0e-6f)
        return result;
    if (sine > 1.0e-6f && angle > 1.0e-6f)
    {
        const PhysicalContactVec3 axis =
            sineAxisTwice * (1.0f / (2.0f * sine));
        angularVelocity = axis * (angle / elapsedSeconds);
    }
    const float angularSpeed = PhysicalContactLength(angularVelocity);
    if (!PhysicalContactFinite(angularVelocity) ||
        !std::isfinite(angularSpeed) ||
        angularSpeed > maximumAngularRadiansPerSecond)
        return result;

    result.valid = true;
    result.linearVelocity = linearVelocity;
    result.angularVelocity = angularVelocity;
    result.pivot = current.position;
    return result;
}
...
PhysicalContactHoldRecentDynamicBodySeparation(
    ...
    int32_t constrainedTargetHandle, uint64_t lastBlockedMs,
    uint64_t nowMs, uint64_t holdMs)
{
    if (observation == PhysicalContactDynamicBodyObservation::Separated &&
        constrainedTargetHandle != -1 && lastBlockedMs &&
        nowMs >= lastBlockedMs && nowMs - lastBlockedMs <= holdMs)
        return PhysicalContactDynamicBodyObservation::Uncertain;
    return observation;
}
    ...
    float configuredThresholdMetersPerSecond, uint8_t targetKind)
{
    ...
    // vehicles, but halve it for bipeds/creatures/giants. At the default
    // 1.50 m/s this is an exact 0.75 m/s enemy hit. It remains well above the
    // 0.05 m/s nudge floor, and the armed latch/cooldown still prevent rubbing
    // a weapon against an enemy from repeating damage.
    return std::clamp(
        configuredThresholdMetersPerSecond * 0.50f, 0.50f, 4.00f);
}
