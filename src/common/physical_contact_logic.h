...
// Find a rigid translation whose final pose is proven clear by the complete
// exact weapon-versus-target query. A selected contact plane is only a useful
// starting distance: concave and multi-part geometry can still overlap another
// face after that one plane is cleared. The final returned pose is always
// checked directly. The fixed search has no allocation and a bounded number of
// exact queries.
template <typename IntersectsAt>
inline PhysicalContactWallConstraint PhysicalContactExactSeparationOffset(
    const PhysicalContactTransform& intendedWeaponTransform,
