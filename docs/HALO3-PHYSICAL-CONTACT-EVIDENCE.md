...
The strict external-visible-state Halo 3 Forge replay on Valhalla still failed.
Its preserved log is
...
After 410 exact visible palettes the run recorded one triangle-surface overlap
and zero solid-compound overlaps. The proposal gate was active
(`approvedPalettes=123`, `heldPalettes=403`), but the target continued moving
and rotating into the held clear pose after native impulses. Holding a
previously clear palette is therefore insufficient by itself. This behavior is
rejected and reverted by `b3c47db`. MCC closed and SteamVR was restored to the
byte-identical normal settings hash
`F8B2C009AE05A2AC796D3458B9AA8B072A5BEA97CA8EC5A74FAA5DD585DBAB9E`.

The next candidate keeps the render proposal/approval boundary and direct
checked release, but reserves motion-dependent clearance before publishing a
blocked pose. It measures the target surface point's full rigid-body velocity
(translation plus angular velocity about the native centre), predicts 60 ms of
motion, and adds that to an 8 mm exact triangle-and-solid clearance, bounded at
35 mm. A still target therefore remains precise, while an already moving or
rotating prop cannot consume the entire worker-to-render safety gap. Invalid
motion data selects the bounded maximum gap. The corrected full palette is
published only after the final candidate directly passes both geometry tests;
the renderer keeps that approval until another exact proof succeeds. Unit tests
cover the still, moving, clamped, invalid-data, and invalid-policy cases. The
strict runtime gate additionally requires at least 250 corrected palettes, so
a pass cannot come from merely holding old uncorrected poses.
